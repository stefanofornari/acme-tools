...
public final class TestUtils {
    ...
    /**
     * Reads the resources available for the given uri from the directory.json
     * config file.
     *
     * @param uri
     * @return a Map containing the uri's resources configuration
     *
     * @throws IOException in case of errors locating/reading directory.json
     */
    public static Map<String, String> directoryFor(final String uri) throws IOException {
        final JSON ALL = TestUtils.getJSON("directory");
        final JSON CA = ALL.get(uri).asObject();
        ...
        for (String key : CA.keySet()) {
            try {
                ...
                //
                RET.put(Resource.valueOf(key).path(), CA.get(key).asString());
            } catch (IllegalArgumentException x) {
                //
                // Othr resources
                //
                RET.put(key, CA.get(key).asString());
            }
        }
